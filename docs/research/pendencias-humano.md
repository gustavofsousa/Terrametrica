...
(`dados-geoniteroi.opendata.arcgis.com`) ou no portal oficial (`sigeo.niteroi.rj.gov.br`).
...
que isso só aconteça bem depois do MVP.

## 4. Push dos commits locais para o remoto

**O quê:** a branch `main` está **6 commits à frente de `origin/main`** (incluindo o commit de
verificação da Fase 0 de hoje). Eu não faço push sem pedido explícito.
**Ação:** `git push` quando você quiser publicar. Aviso: nenhum desses commits é destrutivo, é
seguro fazer a qualquer momento.

## 6. Baixar shapefile SIGEF (Acervo Fundiário RJ) — precisa login GOV.BR

**O quê:** confirmado por navegação real (2026-09-03) que o dataset SIGEF é **ACESSO PÚBLICO,
licença CC-BY**, mas o recurso SHP em `certificacao.incra.gov.br/csv_shp/export_shp.py` exige
login via SSO `sso.acesso.gov.br` — **qualquer conta gov.br de cidadão comum serve**, não é
...
ingestão rural.
**Ação:** logar em `certificacao.incra.gov.br/csv_shp/export_shp.py` com sua conta gov.br, filtrar
por RJ, baixar o shapefile e me passar o arquivo (ou o caminho local) para eu medir feições/CRS/
validade.

## 7. Baixar shapefile CAR (Perímetros dos imóveis, RJ) — precisa resolver reCAPTCHA

**O quê:** confirmado por navegação real (2026-09-03) que
`consultapublica.car.gov.br/publico/estados/downloads` **não exige login** e lista RJ com dado
fresco (disponibilizado 03/09/2026). O modal de download por camada (Perímetros dos imóveis, APP,
Reserva Legal etc.) é protegido por **reCAPTCHA por clique** — não tentei contornar, é controle
anti-bot legítimo.
...
**Ação:** na página acima, clicar RJ → "Perímetros dos imóveis" → Download, resolver o captcha,
baixar o shapefile e me passar o arquivo para eu medir a sobreposição.

## 5. Inventário dos 92 municípios (cobertura urbana)
